...
class SoftmaxRegression(NNBase):
    ...
    def __init__(self, wv, dims=[100, 5],
        ...
        param_dims_sparse = dict(L=wv.shape) # e.g: {'L': (V, d)}
        ...
        self.params.W = random_weight_matrix(*self.params.W.shape) # W is a 5x100 matrix
    ...
    def _acc_grads(self, idx, label):
        ...
        p = softmax(self.params.W.dot(x) + self.params.b) # W:(5,100), x:(100, 1), b:(5, 1)
        ...
        self.sgrads.L[idx] = self.params.W.T.dot(delta) # Reg term only works on parameters W, not b or wv.
